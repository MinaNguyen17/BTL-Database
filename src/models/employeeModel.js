...
async function createEmployee(ID_Card_Num, Fname, Lname, DOB, Sex, Position, Wage) {
	const pool = await getDBConnection();
	try {
...
async function getEmployeeSalary(idCardNum, month = null, year = null) {
	try {
		const pool = await getDBConnection();
		...
		const result = await pool
			...
			.execute("ViewEmployeeSalary");

		return result.recordset; // Trả về danh sách lương
	} catch (error) {
		console.error("Error in getEmployeeSalary:", error);
		throw new Error("Could not retrieve employee salary");
	}
}
